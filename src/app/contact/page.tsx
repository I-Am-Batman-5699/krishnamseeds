...
import { ContactDataRes, ContactInfo, Platforms } from "@/lib/types";
...
export default function Page() {
	...
	const getContent = useCallback(async () => {
		try {
			const response: ContactDataRes = await ApiService.get<ContactDataRes>("/api/data/contacts");
	...
	const fetchData = useCallback(async () => {
		if (appContactData.length === 0 && !appContactDataFetched) {
			try {
				...
				Object.keys(oData.platforms).length > 0 && setAppContactPlatformsData(oData.platforms);
	...
	}, [appContactData.length, appContactDataFetched, getContent]);
	...
	const onEmailSubmit = async (e: any) => {
		e.preventDefault();
		...
		try {
			...
			console.info("Email request successfully sent.");
	...
	const retryFetch = () => {
		setAppContactData([]);
		setAppContactPlatformsData({});
		setAppContactDataFetched(false);
		setFetchError(null);
		fetchData();
	};

	const bgMain = "min-h-[calc(100svh-65svh)] opacity-90";
			...
			<div>
				<section className="w-full min-h-[calc(100svh-60svh)] flex justify-center py-12 md:py-24 lg:py-32 border-t" id="contact">
					<div className="container flex items-center justify-center flex-col gap-4 px-4 text-center md:px-6">
						...
						{appContactDataFetched && appContactData && Object.keys(appContactData).length > 0 && (
							<>
				...
				{!appContactDataFetched && <LoadingSpinner className={bgMain} />}
				{fetchError && (<div>
					<div className="flex flex-col items-center justify-center w-full min-h-[calc(100svh-70svh)] overflow-hidden">
						<p>Error: {fetchError.message}</p>
						<button onClick={retryFetch} className="p-2 bg-blue-500 rounded-md text-white">Retry</button>
					</div>
				</div>)}
				{appContactDataFetched && (<>
